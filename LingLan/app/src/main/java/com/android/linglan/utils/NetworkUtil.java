...
//  }
  ...
  private static AccessType getAccessType(NetworkInfo networkInfo) {
    int networkType = networkInfo.getType();
    AccessType accessType = AccessType.UNKNOWN;
    if (ConnectivityManager.TYPE_WIFI == networkType) {
      accessType = AccessType.WIFI;
    } else if (ConnectivityManager.TYPE_MOBILE == networkType) {
      TelephonyManager telephonyManager = (TelephonyManager) LinglanApplication.getsApplicationContext().getSystemService(
          Context.TELEPHONY_SERVICE);
      switch (telephonyManager.getNetworkType()) {
        case TelephonyManager.NETWORK_TYPE_CDMA:
        case TelephonyManager.NETWORK_TYPE_GPRS:
        case TelephonyManager.NETWORK_TYPE_EDGE:
          accessType = AccessType.SECOND_G;
          break;
        case TelephonyManager.NETWORK_TYPE_UMTS:
        case TelephonyManager.NETWORK_TYPE_HSDPA:
        case TelephonyManager.NETWORK_TYPE_EVDO_A:
        case TelephonyManager.NETWORK_TYPE_EVDO_B:
        case TelephonyManager.NETWORK_TYPE_EVDO_0:
          accessType = AccessType.THIRD_G;
          break;
        case TelephonyManager.NETWORK_TYPE_LTE:
          accessType = AccessType.FOURTH_G;
          break;
      }
    }
    return accessType;
  ...
  public static int getAccessType() {
    ...
    ConnectivityManager connManager = (ConnectivityManager) LinglanApplication.getsApplicationContext().getSystemService(
        Context.CONNECTIVITY_SERVICE);
    if (connManager != null) {
      try {
        // in some rom and a special time, it maybe throw NullPointer ex,
        ...
        accessType = getAccessType(networkInfo);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
...
//      operation = Operation.UNICOM;
//    } else if (imei.startsWith("46003")) {
//      operation = Operation.TELECOM;
//    }
...
//  }
  ...
  public static String getMacAddress() {
    WifiManager wifiManager = (WifiManager) LinglanApplication.getsApplicationContext().getSystemService(Context.WIFI_SERVICE);
    if (wifiManager != null) {
